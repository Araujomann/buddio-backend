...
export async function followRoutes(fastify, options) {
    ...
    fastify.delete(
        ...
        async (req, reply) => {
            ...
            const removeFollower = await Follow.findOneAndDelete({
              followerId,
              followedId: id,
            })

            if (!removeFollower) {
              return reply.status(404).send({ message: "Não foi possível remover esse usuário." })
            }

            return reply.send({ message: "Este usuário não é mais um seguidor." })
